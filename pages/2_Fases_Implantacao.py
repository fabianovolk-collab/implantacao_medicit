...
STATUS_VALIDOS = ["Não iniciado", "Em andamento", "Bloqueado", "Concluido"]
...
CHECKLISTS = {
    ...
    "Go-Live": [
        ...
        "Base importada (se aplicável)",
        "Equipe apta a operar",
        "Data de início definida"
    ],
    "Acompanhamento": [
        "Cliente usando sistema",
        "Primeiros agendamentos realizados",
...
with col1:
    ...
    if doc_busca:
        clientes_filtrados = clientes_df[
            clientes_df["CNPJ_CPF"].str.contains(doc_busca)
        ]
    else:
        clientes_filtrados = clientes_df
...
bloqueado = col2.selectbox("Bloqueado", ["Não", "Sim"], index=0 if dados["Bloqueado"] == "Não" else 1)
responsavel = col3.text_input("Responsável", value=dados["Responsavel_Etapa"])

col4, col5, col6 = st.columns(3)

data_inicio = col4.date_input("Data Inicio", value=parse_data(dados["Data_Inicio"]), format="DD/MM/YYYY")
data_prevista = col5.date_input("Data Prevista", value=parse_data(dados["Data_Prevista"]), format="DD/MM/YYYY")
data_conclusao = col6.date_input("Data Conclusão", value=parse_data(dados["Data_Conclusao"]), format="DD/MM/YYYY")

motivo = st.text_input("Motivo (Bloqueio/Atraso)", value=dados.get("Motivo_Bloqueio", ""))
...
if st.button("💾 Salvar"):
    ...
    if bloqueado == "Sim" and motivo.strip() == "":
        st.warning("Informe o motivo do bloqueio")
        st.stop()
    ...
    df.at[idx, "Bloqueado"] = bloqueado
    df.at[idx, "Motivo_Bloqueio"] = motivo
    ...
    df.at[idx, "Data_Prevista"] = formatar_data(data_prevista)
