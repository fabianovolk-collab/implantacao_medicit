...
import pandas as pd
from database import carregar_clientes, salvar_clientes
...
# ==============================
# MÁSCARAS DINÂMICAS (CORREÇÃO)
# ==============================

st.session_state.documento = formatar_documento(st.session_state.documento)
st.session_state.telefone = formatar_telefone(st.session_state.telefone)
...
with st.form("form_cliente"):
    ...
    tipo_index = 0 if st.session_state.tipo_pessoa == "Pessoa Física" else 1
    col1.selectbox(
        "Tipo de Pessoa",
    ...
    col2.text_input(
        "CPF / CNPJ",
        value=st.session_state.documento,
        key="documento"
    )
    ...
    if st.session_state.modo == "edicao":
        salvar = st.form_submit_button("✏️ Atualizar Cliente")
    else:
